package io.kouna.mcgui.utilities.rendering
...
class SingleEntityRenderer<T : Entity>: Closeable {
    ...
    fun render(part: RenderEntity<T>) {
        entity = (entity ?: io.kouna.mcgui.utilities.spawnEntity(part.location, part.clazz) {
            part.init(it)
        }).apply {
...
class GroupEntityRenderer: Closeable {
    ...
    private fun <T: Entity>renderPart(id: Any, template: RenderEntity<T>) {
        ...
        val entity = io.kouna.mcgui.utilities.spawnEntity(template.location, template.clazz) {
            template.init(it)
        }
        rendered[id] = entity
    }
